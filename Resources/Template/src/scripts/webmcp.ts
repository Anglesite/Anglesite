...
if ("modelContext" in document && document.modelContext) {
  ...
  void registerSafely(FETCH_POST_MARKDOWN_TOOL, async ({ path }: { path: string }) => {
    try {
      ...
      // Covers fetch() rejecting outright (offline, DNS, CORS, etc.) — not just a non-2xx
      // response, which is already handled above.
      console.warn(`[webmcp] fetch failed for ${path}:`, err);
      return { content: [{ type: "text", text: `Failed to fetch: ${path}` }] };
    }
  });
}
