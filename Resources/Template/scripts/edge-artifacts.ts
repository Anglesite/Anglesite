 ...
 * public/.well-known/atproto-did, public/.well-known/site.standard.publication, and
 * public/.well-known/http-message-signatures-directory. Runs at
...
/**
 * HTTP Message Signatures Directory (#1581, closing the `webBotAuth` Agent Readiness gap;
 * draft-meunier-http-message-signatures-directory) at `.well-known/http-message-signatures-
 * directory`. This directory is published by whoever *signs* outbound requests (RFC 9421) so a
 * recipient can verify them — it is not how a site verifies *inbound* bot traffic, which
 * Cloudflare already handles at the edge via its own Verified Bots feature. Anglesite sites don't
 * sign any outbound requests today, so there is no real key material to publish; this generator
 * emits an empty, spec-valid JWKS (RFC 7517 §5) purely so the well-known path resolves instead of
 * 404ing. No mode/config: unlike security.txt/mta-sts.txt this needs no owner input and is always
 * on, the same way `mcpServerCard` needs none beyond `experimental.mcp` — here there isn't even a
 * feature flag, since an empty key set is always correct until real outbound signing exists.
 */
export const HTTP_MESSAGE_SIGNATURES_DIRECTORY_MARKER = "__anglesite_generated_http_message_signatures_directory__";

/** True when `content` parses as JSON and carries Anglesite's generated-output marker field —
 *  mirrors `isMcpServerCardMarkerOwned` above. */
export function isHttpMessageSignaturesDirectoryMarkerOwned(content: string | null): boolean {
  if (content === null) return false;
  try {
    const parsed = JSON.parse(content) as Record<string, unknown>;
    return parsed.__marker === HTTP_MESSAGE_SIGNATURES_DIRECTORY_MARKER;
  } catch {
    return false;
  }
}

/** An empty JWKS: no keys, because Anglesite sites don't yet sign outbound requests. */
export function buildHttpMessageSignaturesDirectory(): string {
  return JSON.stringify({ __marker: HTTP_MESSAGE_SIGNATURES_DIRECTORY_MARKER, keys: [] }, null, 2) + "\n";
}

export type HttpMessageSignaturesDirectoryAction = { kind: "write"; content: string } | { kind: "none" };

export interface HttpMessageSignaturesDirectoryPlan {
  action: HttpMessageSignaturesDirectoryAction;
  note?: string;
}

/** Always writes the empty directory, unless a hand-authored (non-marker) file already occupies
 *  the path — matching `planMcpServerCard`'s refuse-to-overwrite posture. */
export function planHttpMessageSignaturesDirectory(
  existingContent: string | null,
): HttpMessageSignaturesDirectoryPlan {
  if (existingContent !== null && !isHttpMessageSignaturesDirectoryMarkerOwned(existingContent)) {
    return {
      action: { kind: "none" },
      note:
        "public/.well-known/http-message-signatures-directory already exists and wasn't generated by " +
        "Anglesite — refusing to overwrite it. Delete the file to let Anglesite generate it.",
    };
  }
  return { action: { kind: "write", content: buildHttpMessageSignaturesDirectory() } };
}
...
function applyHttpMessageSignaturesDirectoryPlan(publicDir: string): void {
  const wellKnownDir = resolve(publicDir, ".well-known");
  const filePath = resolve(wellKnownDir, "http-message-signatures-directory");
  const existingContent = existsSync(filePath) ? readFileSync(filePath, "utf-8") : null;
  const plan = planHttpMessageSignaturesDirectory(existingContent);
  if (plan.note) console.log(plan.note);
  if (plan.action.kind === "write") {
    mkdirSync(wellKnownDir, { recursive: true });
    writeFileSync(filePath, plan.action.content, "utf-8");
    console.log("Wrote public/.well-known/http-message-signatures-directory");
  }
}
...
function main(): void {
  ...
  applyHttpMessageSignaturesDirectoryPlan(publicDir);
