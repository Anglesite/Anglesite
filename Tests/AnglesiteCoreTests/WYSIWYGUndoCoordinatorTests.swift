...
struct WYSIWYGUndoCoordinatorTests {
    ...
    func doesNotCoalesceAcrossSeparateSessions() async {
        ...
        undoManager.undo()
        await coordinator.pendingPerform?.value
        #expect(performed == [WYSIWYGOpInverter.invert(op2)])
        #expect(undoManager.canUndo == true)

        undoManager.undo()
        await coordinator.pendingPerform?.value
        #expect(performed == [WYSIWYGOpInverter.invert(op2), WYSIWYGOpInverter.invert(op1)])
    }
    ...
    func doesNotCoalesceAcrossDifferentBlocks() async {
        ...
        coordinator.registerApplied(op: opA, inverse: WYSIWYGOpInverter.invert(opA))
        let opB = Op.editText(blockId: "b", runs: [RichTextRun(kind: .text, text: "Hi you")], previousRuns: baseline)
        coordinator.registerApplied(op: opB, inverse: WYSIWYGOpInverter.invert(opB))

        undoManager.undo()
        await coordinator.pendingPerform?.value
        #expect(performed == [WYSIWYGOpInverter.invert(opB)])
    ...
    func coalescingStopsAfterUndoRedo() async {
        ...
        undoManager.undo()
        await coordinator.pendingPerform?.value
        #expect(performed.last == WYSIWYGOpInverter.invert(op2))
