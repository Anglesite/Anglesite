...
`Resources/container-image/` is a gitignored local build artifact, so it's never rebuilt automatically — if your sidecar checkout was already vendored before pulling an app change that bumps the required MCP protocol version, the image quietly keeps serving the old one. `scripts/lib/stage-dev-image-context.sh` guards against the current known case (the MCP 2026-07-28 stateless spec, #1277): both vendor scripts refuse to stage a sidecar checkout older than the pinned minimum version and tell you how to update it. When in doubt, `git -C ../anglesite pull` and re-run the vendor script before debugging an MCP failure.
