...
- **Native Desktop Application**: Full Electron application with native menu system
- **Automatic HTTPS Support**: Local SSL certificates with trusted CA installation (no admin required)
- **Smart DNS Management**: Automatic .test domain configuration via /etc/hosts with Touch ID support
...
- **Biometric Authentication**: Touch ID support for macOS hosts file modifications with intelligent fallback
- **Enhanced Website Management**: Real-time validation, in-place renaming, and context menus
...
2. **Certificate Installation** (HTTPS mode only):
   ...
   - Uses mkcert for secure, self-contained certificate generation
