...
// `EngineEvent` deliberately does NOT live here. It composes op-queue.ts's `AppliedEvent`/
// `RejectedEvent` rather than restating their shape, and op-queue.ts imports this file — so it is
// declared in engine.ts (which already depends on both) and re-exported from src/index.ts.
