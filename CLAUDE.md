...
- **Remove your worktree the moment its PR merges or the work is abandoned** — `git worktree remove <path>` deletes the checkout and its `.build` in one step. Left in place, a worktree's `.build` alone commonly grows past 5GB, and since each worktree has its own `Anglesite.xcodeproj` path, each also leaves Xcode a distinct `~/Library/Developer/Xcode/DerivedData/Anglesite-*` folder that nothing ever reclaims. Confirmed the hard way: a machine with a few dozen stale worktrees had burned 250GB+ this way before anyone noticed (2026-08). `DerivedData` itself is disposable — safe to `rm -rf ~/Library/Developer/Xcode/DerivedData/*` wholesale when reclaiming space — but not mid-`sync` of the localization catalog (see `CONTRIBUTING.md` ▸ the `.stringsdata`/`sync` recipe), which reads from whatever DerivedData your machine already accumulated.
- **Before removing a worktree you didn't create this session, verify it's actually safe**: `git status --short` inside it must be clean (or its changes are already captured in the merged/closed PR), *and* no live process has it as a cwd — `lsof -p <pid> -a -d cwd` for any `claude`/agent/build process, since another agent can be using a worktree between commits even while its git state looks quiet. If `git worktree remove` refuses with "contains modified or untracked files," that's the safety check doing its job — look at what changed before ever reaching for `--force`. Confirmed the hard way: force-removing a worktree that only *looked* idle pulled the working directory out from under a live agent session mid-task.
- **`.claude/worktrees/` on disk and `git worktree list` can drift out of sync** — a directory rename, a repo relocation, or an interrupted removal can leave a worktree directory whose `.git` file points at a `gitdir` that no longer resolves to anything. `comm -23 <(ls .claude/worktrees | sort) <(git worktree list --porcelain | awk '/^worktree/{n=$2; sub(/.*worktrees\//,"",n); print n}' | sort)` surfaces these. Anything it lists isn't a valid worktree — `git worktree remove` won't touch it — so it's a plain leftover directory; apply the same live-process check above, then `rm -rf` it directly.
...
Tests: `swift test --package-path .` runs the SwiftPM test targets (`AnglesiteSiteModelTests`, `AnglesiteCoreTests`, `AnglesiteBridgeTests`, and, on Swift 6.4+/Xcode 27, `AnglesiteIntentsTests`). The non-Mac app and extension targets are covered the same way (#1968, D2): each keeps its views/entry points in the Xcode target and its logic in a thin SwiftPM library with a test target beside it — `AnglesiteMobileCore`/`AnglesiteMobileCoreTests` (iOS shell), `AnglesiteQuickLookUI`/`AnglesiteQuickLookUITests` (Quick Look preview + thumbnail, rendered against the committed `Tests/AnglesiteQuickLookUITests/Fixtures/Fixture.anglesite`), `AnglesiteShareExtensionCore`/`AnglesiteShareExtensionCoreTests`, and, Linux-only, `AnglesiteLinuxCore`/`AnglesiteLinuxTests` (run in the `linux-build-test` lane, or locally in the `swift:6.3.3-noble` podman container per `CONTRIBUTING.md`). Put new iOS/extension/Linux-shell logic in those libraries, not in the Xcode targets. `AnglesiteContainerLocalTests` is opt-in with `ANGLESITE_CONTAINER_TESTS=1`; its end-to-end cases also require `ANGLESITE_CONTAINER_E2E=1`. All suites are Swift Testing (#74, #1812). The MCP / apply-edit e2e tests (`AppliesEditEndToEndTests`, `MCPClientHTTPEndToEndTests`) need the sibling sidecar checkout + node; they're gated with Swift Testing's `.enabled(if:)` trait, so they skip cleanly when the checkout is absent — set `ANGLESITE_PLUGIN_PATH` to the sidecar checkout to make them run. If `swift build`/`swift test` seems to hang with no output, a stale SwiftPM process is likely holding the `.build` lock — check `pgrep -fl swift-test` and kill the orphan rather than assuming a bad test. **Run local full suites through `scripts/swift-test.sh`** (same arguments as `swift test`): it holds a machine-scoped lock (`/tmp/anglesite-swift-test.lock`, a `mkdir` directory) so two agents' full runs on one Mac serialize instead of colliding — on-device FoundationModels inference is a system-wide FIFO queue across all processes (#1594), so concurrent runs multiply each live-model turn's wall clock past the suites' timeouts. Compilation and `--filter` runs of non-model suites don't need the lock.
...
`gh issue list` is the source of truth for what to work on, with [`docs/build-plan.md`](docs/build-plan.md) for the phased roadmap and the per-epic status snapshot (Containerization, Claude Code removal, Component Editor, Personal Publishing OS, cross-platform port, and the rest). Issue numbers in that snapshot are illustrative and may be stale (many are already closed) — confirm against gh before picking up work. Current phase: **Phase 10** — v2 polish (tracking: #34). Phases 0–9 are complete. v1.0 is gated on App Store submission (#617) *and* the software-factory (#1256) and WYSIWYG (#1221) epics, plus overlay removal (#1957) and the gate-hash pin (#1958) (decision D8). The `v0.5.0-Alpha` and `v1.0.0-Beta` milestones were closed on 2026-09-08 and their open items folded into `v1.0` (#1967); `v1.0` and `Future` are the only open milestones.

**Design docs are indexed.** [`docs/specs/README.md`](docs/specs/README.md) and [`docs/superpowers/README.md`](docs/superpowers/README.md) list every spec, plan and decision record with its date, issue and status (generated by `scripts/generate-docs-index.py`; CI fails when stale). Check the **Status** column before treating a spec as current — `superseded by …` names the replacement — and give every new design doc the one-line `Status:` header described in `CONTRIBUTING.md` ▸ "Design docs" (#1816).

**Issue-in-flight signaling.** Multiple agents work this repo concurrently (see "Worktrees" above), so before starting work on a tracked issue, check it isn't already claimed and mark that you're taking it: `gh issue list --label "🛠️ In Progress"` to check, then `gh issue edit <n> --add-label "🛠️ In Progress"` to claim it. Leave the label in place once the PR is open — don't remove it. A PR body with a closing keyword (`Closes #<n>` / `Fixes #<n>` — see `CONTRIBUTING.md` ▸ "Commits and pull requests") closes the issue on merge, and `gh issue list` only shows open issues by default, so a closed issue naturally drops out of the claimed-issue check without any manual label cleanup. If you find an issue already fixed/merged before you could start (as happens — two agents can pick the same issue in the same window), don't silently redo the work: check for an existing PR/commit first, and if one already landed, close the issue referencing it rather than duplicating the fix.
