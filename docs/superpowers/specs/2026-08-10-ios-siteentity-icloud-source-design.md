...
| Cross-target dependency | `AnglesiteIntents` gains a `.when(platforms: [.iOS])`-conditioned dependency on `AnglesiteIOS` in `Package.swift`, rather than moving `UbiquitousPackageDiscovering`/`NSMetadataQueryPackageDiscovery` to `AnglesiteCore` | Precedented: `Package.swift` already uses `.when(platforms: [.macOS])` for macOS-only dependency edges elsewhere in this exact file. No file relocation needed, and `AnglesiteIOS` has no dependency back on `AnglesiteIntents`, so this doesn't create a cycle. |
