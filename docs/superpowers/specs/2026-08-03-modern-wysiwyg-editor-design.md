...
**Amended 2026-09-08:** the block editor is the only editing surface; `JS/edit-overlay` click-to-edit is deprecated and removed before 1.0 (owner decision D4, `docs/specs/2026-09-08-product-direction-review-decisions.md`). **Executed in #1957:** the overlay is gone, the block canvas is the site window's default state, and the overlay's page-level reporters (Siri visible-elements, Effects/experiment pick modes, the Component Editor canvas) now ride the engine bundle's page bridge on the single `wysiwyg` namespace.
**Related:** Component Editor (`2026-07-05-component-editor-design.md`, epic #496 — shipped), the retired edit overlay (`JS/edit-overlay/`, removed in #1957), #459 (deterministic-path direction), #571 (cross-platform port), #72 (git is the source of truth), `docs/mac-assed-app-spec.md` (platform UX standard for the first host)
...
1. ~~Decide the landing target~~ **Decided: Anglesite** — the block editor
   supersedes the route click-to-edit overlay. **Binding and done (D4, #1957):**
   the overlay bundle, its `anglesite` script-message namespace, and the
   git-revert `EditUndoCoordinator` are removed; the block canvas mounts by
   default and follows every preview navigation; hover outline and image drop
   onto an `<img>` live in `JS/wysiwyg-engine/src/host/`.
