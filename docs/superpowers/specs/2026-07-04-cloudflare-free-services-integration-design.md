...
2. **Resolve the zone, then the Bot Fight Mode consequences question.** Immediately after resolving the domain to a zone ID, read the zone's state (`CloudflareReading.zoneState`). If Bot Fight Mode is on, stop and ask the site owner directly, phrased about consequences to their site (not mechanics): Bot Fight Mode is protecting the site from automated traffic, but it also blocks the AI Search crawler, and there is no way to make an exception for just that crawler. The owner chooses "Keep It On" (proceeds; AI Search may not be able to crawl) or "Turn Off Bot Fight Mode" (calls `CloudflareWriting.setBotFightMode(zoneID:enabled:false,…)`, then proceeds) before either path reaches the cost disclosure below. This replaces an earlier design that planned a WAF skip rule, applied after provisioning, to exempt the crawler from Bot Fight Mode — live verification against the real Cloudflare API (2026-08-15) found that approach can never work (see §15's 2026-08-15 entry for the three independent reasons). There is no code change to `HardenPlanner.plan(from:domain:)`'s shared signature either way — this step, like the rest of Slice 8, is a self-contained call in the AI Search flow.
3. **Preflight — cost and model disclosure.** Unlike every other slice in this doc, Workers AI/AI Gateway billing scales with reader traffic rather than being flat-free or one-time — a popular page can push a site past the free tier (10k neurons/day, then metered). Show an explicit at-cost disclosure requiring confirmation before provisioning, in the same spirit as Slice 7's purchase confirmation but for a metered cost. The same step states plainly that reader queries run on Cloudflare's models — this doesn't implicate the app-side #459 on-device policy (which governs app features, not published-site behavior) but the disclosure belongs here regardless.
4. **Provision the AI Search instance** via REST (`POST /accounts/{account_id}/ai-search/instances`, flat path, body `{"id","type":"web-crawler","source"}` — live-verified 2026-08-15; the API reference's namespaced `/ai-search/namespaces/{name}/instances` path is a separate, pre-created-namespace workflow). No capability-gating framework exists yet for *any* slice (`CloudflareCapabilityProber`/`TokenCapabilities` have zero consumers today, and no "missing capability → upgrade prompt" UI exists to plug into) — this slice doesn't invent that machinery either. It attempts the call directly; a 403/permission error surfaces a friendly message pointing at token settings, matching how the rest of the app handles permission errors today. The Slice 0 token template documentation (Section 4) still lists `AI Search (Edit/Account)` as a permission to add when a token is (re)created, since that's just guidance text, not code this slice depends on.
5. **NLWeb dashboard handoff.** NLWeb enablement has no API and is public preview as of 2026-07 — the flow ends with a "finish setup in the Cloudflare dashboard" step: a deep link to the AI Search product page (`https://dash.cloudflare.com/?to=/:account/ai/ai-search` — the docs-verified pattern as of 2026-08-15; note the `/ai/` segment, and no per-instance settings deep link exists in Cloudflare's docs), plus the manual steps listed inline (select the instance, open Settings, locate "NLWeb Worker", enable it). Same shape as the iCloud-email handoff in Slice 2.
...
**Testing:** hand-rolled `CloudflareReading`/`CloudflareWriting` mocks for instance-provisioning and the Bot Fight Mode toggle, matching the `MockCloudflareReader`/`MockCloudflareWriter` pattern in `Tests/AnglesiteCoreTests/HardenExecutorTests.swift`; Swift Testing (`@Test`/`#expect`, not XCTest, per the rest of `AnglesiteCoreTests`); a unit test asserting the flow refuses to proceed when `aiInput == "no"`, and proceeds when it's `"unset"` or `"yes"`; coverage for both branches of the Bot Fight Mode decision — "Turn Off Bot Fight Mode" calls `setBotFightMode(enabled: false)` and proceeds, "Keep It On" proceeds without calling the writer and records that choice for the final success screen; live e2e behind `ANGLESITE_CF_E2E=1` like the rest.
...
Slices 1–7 all depend on Slice 0 (its capability-gating framework, once one exists — see Section 12's note that no slice has built this yet). Slice 2's contact form depends on Slice 1 (Turnstile). Slice 8 is independent of every other slice: it's self-contained (own provisioning call, own Bot Fight Mode decision step, existing `LicensingStore`) rather than extending shared planning code, so it doesn't depend on Slice 0 landing first or on Slice 3's Harden-pack work. Everything else can reorder opportunistically.

Each slice gets its own implementation plan (`docs/superpowers/plans/`) when picked up, per the #242/#459 convention.

## 15. API facts verified (2026-07-04, Slice 8 facts added 2026-07-30, WAF-skip-rule correction 2026-08-15)
...
- **WAF skip rule correction, live-verified 2026-08-15 (real token against the real API):** an earlier version of Slice 8 planned a WAF custom rule with `action: "skip"` to exempt the AI Search crawler from Bot Fight Mode. This is impossible, for three independent reasons: (1) `action_parameters.products: ["botFight"]` is rejected by the API — error 20119, "skip action parameter product 'botFight' is invalid" — on every variant tried; (2) the alternative match field, `cf.bot_management.detection_ids`, isn't entitled on free zones ("a Bot Management plan is required"); (3) even a rule that validated would do nothing, because Cloudflare's own docs (bots/get-started/bot-fight-mode) state that Bot Fight Mode evaluates in a separate pipeline where WAF custom-rule Skip/Bypass/Allow actions have no effect on it. (`cf.client.bot` is usable on free plans, but is irrelevant here since no WAF rule can affect Bot Fight Mode either way.) The only real remediation is turning Bot Fight Mode off, which is a genuine owner trade-off — so Slice 8's flow step 4 now asks the owner directly instead of attempting a rule that can't work.
