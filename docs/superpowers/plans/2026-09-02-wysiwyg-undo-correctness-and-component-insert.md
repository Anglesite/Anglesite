...
3. **Item 2's fix inherits the sidecar's two documented v1 inverse gaps, unaddressed.** `docs/superpowers/plans/2026-08-19-wysiwyg-sidecar-backed-transport.md`'s design decision 1 (now reversed by this plan) named two known gaps in the sidecar's server-computed `inverse` as reasons it was originally left unconsumed: pruned-import restoration (undoing a `deleteBlock` that removed the last instance of a manifest component may restore the markup without restoring the frontmatter `import` that got pruned alongside it — producing a broken page from an *undo*) and whitespace fidelity on multi-line markup (a non-byte-identical restore changes the file's content hash, which can invalidate a subsequent wire inverse's stamped `baseVersion` and surface as the stale-rejection path Task 7's fix wave added logging for). Fixing either requires a sidecar-side schema change (a paired PR, per `CONTRIBUTING.md` ▸ "Paired PRs") — out of scope here. The manual smoke test in Task 8 includes a step to observe this directly rather than leave it undiscovered until an owner hits it in practice.
4. **Item 2's fix self-corrects going forward, but does not solve every theoretically possible interleaving.** `UndoManager.registerUndo`'s registration must happen synchronously, before any `await` (`WYSIWYGUndoCoordinator.register`'s existing doc comment — verified empirically that `canRedo` never flips true otherwise). That means the *optimistic* re-registration on each fire can't wait for the just-fired `perform` to resolve before deciding what "the next step" is. This plan's answer: register optimistically as before (using the previous fire's `Op`/`WireInverse`, same as today), then — once `perform` resolves — if it reports a fresh, server-computed `WireInverse` for what it just did, swap out the optimistic registration for a corrected one (same `removeAllActions(withTarget:)` + re-register pattern the existing failure-rollback path already uses). Every WYSIWYG op family the sidecar's `component-structure-edit.mjs`/`design-token-edit.mjs`/`text-run-edit.mjs` handle (`insertBlock`, `deleteBlock`, `moveBlock`, `setProp`, `editText`, `setDesignToken`) computes an inverse unconditionally, so in practice this chain keeps self-healing across an entire undo/redo session once the first entry gets a server-computed reversal. What it does **not** fix: the untouched "redo the original op" slot of a registration is never itself corrected mid-flight — only the slot the just-completed `perform` reported on gets refreshed. This is a bounded, named residual gap (inherent to id-addressed editing without OT/CRDT), not a claim of perfect correctness under arbitrary interleavings.
5. **`OpResult`'s wire shape is untouched.** `OpResult` is `Codable` and crosses the JS↔Swift bridge (`WYSIWYGCanvasController.sendOp(_:)`'s JS-facing conformance). Rather than adding a field to it (which would ripple into `JS/wysiwyg-engine/src/types.ts` for no JS-side benefit — JS never needs to know about a server-computed reversal, since undo/redo is entirely native), the new capability (`WYSIWYGServerInvertibleTransport`) is a separate, additive Swift-only protocol that `SidecarWYSIWYGHostTransport` conforms to and `StubWYSIWYGHostTransport` does not; callers detect it with `as?` and fall back to `WYSIWYGOpInverter`'s client-computed inverse when the cast fails.
...
- The sidecar's two v1 inverse gaps — pruned-import restoration, whitespace fidelity (design decision 3).
- Full arbitrary-depth undo/redo correctness under intervening edits (design decision 4).
- Any change to `JS/wysiwyg-engine` (Global Constraints; a type-parity addition to `types.ts` for `manifestName` landed as a final-review fix-wave item, not a behavior change).
...
4. Undo several edits in a row (3+), interleaved with at least one redo, watching for any edit that silently fails to apply or reports an unexpected error — this is the residual-gap case named in this plan's design decision 4; a failure here should be logged as a new, narrower follow-up issue rather than blocking this PR, unless it reproduces the ORIGINAL reported bug (undo restoring to empty content, or addressing a nonexistent node) rather than a new edge case.
5. Delete the ONLY instance of a manifest-registered component block on a page, then ⌘Z — this exercises design decision 3's named, out-of-scope sidecar v1 gap (pruned-import restoration). Confirm whether the frontmatter `import` for that component was pruned and not restored (expected, given the gap) — if the preview shows a broken-import error after this undo, note it in the PR body as a known limitation pointing at this plan's design decision 3, rather than treating it as a surprise regression.
