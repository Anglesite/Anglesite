...
Note: `TypedContentEditor.read`/`write` operate on frontmatter YAML strings only — `FrontmatterDocument` (`Sources/AnglesiteCore/FrontmatterDocument.swift`) parses YAML frontmatter and nothing else, never JSON. The JSON singleton format `ContentScaffold.renderSingleton` produces is a separate, currently-unbridged format; this test exercises the shared `TypedContentEditor` machinery against the real descriptor's shape by hand-writing a YAML fixture, not by feeding it `ContentScaffold.renderSingleton`'s JSON output. This is intentionally the same pattern `TypedContentEditorTests.swift`'s existing `.objectArray` tests already use with a synthetic `experience` fixture (`TypedContentEditorTests.swift:147-183`) — this test's only difference is using the *real* registered descriptor instead of an ad hoc one, so a future edit to the `resume` descriptor's field names is caught here.
...
- [ ] Editable via the generic typed-content editor with no bespoke SwiftUI — the descriptor and `ObjectArrayEditor` are ready, but reaching ANY `src/data`-backed singleton (including the pre-existing `personalProfile`) has no UI path in the app today: no `src/data` visibility in `SiteFileTree`, no content-graph resolution, no caller of `NativeContentOperations.createTypedSingleton` anywhere in `Sources/AnglesiteApp`. This is a pre-existing platform gap this plan doesn't attempt to close, and should be tracked as a separate follow-up.
