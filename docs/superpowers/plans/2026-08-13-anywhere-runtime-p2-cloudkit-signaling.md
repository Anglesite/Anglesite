...
- Code lands on the current worktree/branch (`.claude/worktrees/issue-1208-cef9a3`, branch `claude/issue-1208-cef9a3`). This supersedes an earlier attempt on a now-abandoned worktree (`1208-p2-cloudkit-signaling`, branch `worktree-1208-p2-cloudkit-signaling`, cut from an older `main`) — that worktree's own uncommitted Task 1 diff was discarded as stale (it predates the P1 helper-edit-persistence follow-up, commit `34765e0e`, which touched the same file); only its two docs-only design-spec commits were cherry-picked forward. Task 1 starts from this worktree's current `Sources/anglesite-remote-helper/main.swift`, which already includes that persistence follow-up. Run `xcodegen generate` after any `project.yml` edit (Task 8 adds Settings UI files; verify whether `SettingsView.swift` needs a `project.yml` source-list update — it likely doesn't, being an existing file, but a *new* file for the QR pane does).
...
- [ ] **Step 1: Read the current file in full** (`Sources/anglesite-remote-helper/main.swift`, 145 lines as of this plan's landing on a fresh worktree) before editing — it has evolved twice since P1 shipped: once with `RemoteSiteIdentity.siteID(forSourceDirectory:)` and `DispatchSource`-based SIGTERM handling, and again via the helper-edit-persistence follow-up (commit `34765e0e`, PR #1444), which added the `let control = ContainerizationControl()` local and the `isOwner(siteID:)`-branched `mcpHandler`/`HelperEditPersister` construction between the `httpBridge`/`mcpBridge` lines and the `mcpResponder` line. **The Step 2 code block below predates that second evolution and is stale on those specific lines** — do not paste it verbatim. Do not work from a stale copy.

- [ ] **Step 2: Wrap the existing top-level logic in an `async` function, driven by a minimal `NSApplicationDelegate`.**

Replace the whole file's structure (keeping every line of existing logic — login-item registration, arg parsing, container boot, the `remote-helper: container ready`/`peer connected` markers, `WebRTCPeer.connect`, the SIGTERM `DispatchSource`, the bridge `async let`s, `containerSession.tearDown`) with the shape below. The block is illustrative of the *transformation* (top-level statements moved inside `runSession()`, wrapped by an `NSApplicationDelegate`, `app.run()` at file scope) — apply that same transformation to the file's actual current contents, which additionally includes the `control` local and the `isOwner`/`HelperEditPersister` branch between `mcpBridge` and `mcpResponder` (Step 1). Every line of that branch must survive the transformation unchanged, just re-indented one level inside `runSession()`:
