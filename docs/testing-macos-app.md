...
| `scripts/swift-test.sh` sits at "waiting for swift test lock held by …" | Another full run holds `/tmp/anglesite-swift-test.lock` — expected; a *stale* lock (dead holder pid) is reclaimed automatically | Wait, or `ANGLESITE_TEST_LOCK_WAIT=fail` to bail. A bare `mkdir` lock with no `holder` file and no live `swift test` anywhere (`pgrep -fl swift-test`) was left by a killed hand-rolled run: `rmdir` it |
