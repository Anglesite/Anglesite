...
public actor DeployCommand {
    ...
    /// `package` access looked sufficient (`AnglesiteApp`/`AnglesiteCore` share one SwiftPM
    /// package) but doesn't actually reach `AnglesiteApp` sources compiled through the Xcode
    /// project (`Anglesite.xcodeproj`, generated by XcodeGen) rather than as a SwiftPM product —
    /// confirmed the hard way: `swift build --package-path .` accepted `package` scope, but
    /// `xcodebuild` failed `DeployModel.swift:884` with "'executor' is inaccessible due to
    /// 'package' protection level". Only `swift test`/`swift build` exercise the SwiftPM graph;
    /// the app itself always builds through Xcode, so `public` is the level that actually reaches
