// Sources/AnglesiteCore/AI/NativeContentOperations.swift
import Foundation
#if canImport(Darwin)
import SwiftGit2
#endif
...
public struct NativeContentOperations: ContentOperationsService {
    ...
    #if !canImport(Darwin)
    /// Shared subprocess-`git` runner for the non-Darwin commit/delete paths below — replaces
    /// three identical nested `run(_:)` closures that `processGitCommit`, `processGitCommitPaths`,
    /// and `processGitDelete` each redefined.
    @Sendable private static func runGit(
    ...
    @Sendable public static func processGitCommit(_ projectRoot: URL, _ relPath: String, _ message: String) async -> String? {
        ...
        guard await runGit(["rev-parse", "--git-dir"], git: git, currentDirectoryURL: projectRoot) != nil,
              await runGit(["add", "--", relPath], git: git, currentDirectoryURL: projectRoot) != nil,
              await runGit(["commit", "-m", message, "--", relPath], git: git, currentDirectoryURL: projectRoot) != nil,
              let head = await runGit(["rev-parse", "HEAD"], git: git, currentDirectoryURL: projectRoot)
        else { return nil }
    ...
    @Sendable public static func processGitCommitPaths(
        ...
        let git = URL(fileURLWithPath: "/usr/bin/git")
        guard await runGit(["rev-parse", "--git-dir"], git: git, currentDirectoryURL: projectRoot) != nil,
              await runGit(["add", "--"] + relPaths, git: git, currentDirectoryURL: projectRoot) != nil,
              await runGit(["commit", "-m", message, "--"] + relPaths, git: git, currentDirectoryURL: projectRoot) != nil,
              let head = await runGit(["rev-parse", "HEAD"], git: git, currentDirectoryURL: projectRoot)
        else { return nil }
        return head.stdout.trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    @Sendable public static func processGitDelete(_ projectRoot: URL, _ relPath: String, _ message: String) async -> String? {
        ...
        func run(_ args: [String]) async -> ProcessSupervisor.RunResult? {
            await runGit(args, git: git, currentDirectoryURL: projectRoot)
        }
