...
public enum GitHubTokenVerifyError: Error, Equatable, Sendable {
    ...
    /// Administration + Pages read/write — Pages added for GitHub Pages publishing, #1015 slice
    /// 2c — plus read-only Repository security advisories and Dependabot alerts for #975) so the
    /// user can fix it without leaving the prompt.
    public var userMessage: String {
        switch self {
        case .invalidToken:
            return "That token didn’t work. Create a fine-grained token scoped to All repositories with Contents: Read and write, Administration: Read and write, Pages: Read and write, Repository security advisories: Read, and Dependabot alerts: Read access at github.com/settings/tokens and paste the whole token."
        case .network:
