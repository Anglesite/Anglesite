...
///
/// Phase 4 step 1 shipped the WKWebView configuration tuned for previewing a local Astro dev
/// server; step 2 (#16) registered the JS → native script-message handler on that same
/// configuration. Since #1957 there is exactly one injected script — the block editor bundle
/// (`wysiwyg-engine/engine.js`, which also carries the always-on page bridge) — and one
/// script-message namespace, `wysiwyg`.
public enum WebViewBridge {
    /// The script-message namespace, shared with every platform adapter — see
    /// `WYSIWYGOpsDispatcher.scriptMessageNamespace`. Re-exported here so `AnglesiteApp`'s
    /// `PreviewView` can register/unregister its `WKScriptMessageHandler` from `updateNSView`
    /// without needing a direct `AnglesiteBridgeCore` package dependency.
    public static let scriptMessageNamespace = WYSIWYGOpsDispatcher.scriptMessageNamespace
    ...
    /// the user-content controller under the `wysiwyg` namespace — the JS → native channel for
    /// the block engine's ops protocol and the page bridge's reports (Siri visible elements,
    /// Component Editor canvas, Effects/experiment picks). The engine bundle
    /// (`wysiwyg-engine/engine.js`, built by `scripts/build-wysiwyg-engine.sh`) is injected via
    /// `WKUserScript` at `atDocumentEnd` when present; a missing bundle is non-fatal — the preview
    /// just loads without edit affordances. The configuration is also opted into the full inline
    /// Writing Tools experience (#91, see `enableWritingTools`) so Apple Intelligence's rewrite /
    /// proofread / tone / summarize popover is offered in editable text.
    ///
    ...
    public static func localDevConfiguration(
        ...
        if let script = makeEngineUserScript(in: bundle) {
            config.userContentController.addUserScript(script)
        }
        return config
    }

    /// Loads the bundled block-editor engine (built by `scripts/build-wysiwyg-engine.sh`) as a
    /// `WKUserScript`, or returns `nil` when the bundle hasn't been produced (e.g. `swift test`,
    /// or a build where the prebuild script was skipped). The lookup + read is shared with every
    /// platform adapter via `AnglesiteWysiwygEngineBundle`; only the `WKUserScript` wrapping is
    /// WKWebView-specific.
    @MainActor
    public static func makeEngineUserScript(in bundle: Bundle = .main) -> WKUserScript? {
        guard let source = AnglesiteWysiwygEngineBundle.source(in: bundle) else { return nil }
    ...
    @MainActor
    public static func makeEngineUserScript(from url: URL) -> WKUserScript? {
    ...
    /// *editable* web content (`contenteditable` / `<input>` / `<textarea>`): when the block
    /// editor's `RichTextEditor` (`JS/wysiwyg-engine/src/rich-text.ts`) promotes a text block to
    /// `contentEditable = "true"`, selecting text inside it offers the popover. The rewrite is
    /// applied **inline to the DOM**, so it flows back through the existing pipeline unchanged —
    /// the editor's `input`/`blur` handling captures the rewritten runs as an `editText` op and
    /// lands it through the sidecar as a single undoable commit. No new message type is needed.
    ///
