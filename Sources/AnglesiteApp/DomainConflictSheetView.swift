        ...
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 4) {
                Text("Custom domain already in use")
                    .font(.headline)
                Text("“\(hostname)” is already connected to another site (\(ownedBy)). The site is live at its workers.dev address, but won't use \(hostname) until that's resolved in your Cloudflare dashboard.")
                    .font(.callout)
