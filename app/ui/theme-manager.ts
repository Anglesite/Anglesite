...
class ThemeManager {
  ...
  private setupSystemThemeListener(): void {
    nativeTheme.on('updated', () => {
      console.log('System theme changed to:', nativeTheme.shouldUseDarkColors ? 'dark' : 'light');
      this.updateResolvedTheme();
    });
  }
  ...
  private setupIpcHandlers(): void {
    ipcMain.handle('get-current-theme', () => {
      return {
        userPreference: this.store.get('theme'),
  ...
  private updateResolvedTheme(): void {
    ...
    if (newResolvedTheme !== this.currentResolvedTheme) {
      console.log('Resolved theme changed from', this.currentResolvedTheme, 'to', newResolvedTheme);
  ...
  applyThemeToWindow(window: BrowserWindow): void {
    ...
    const themeScript = `
      (function() {
        ...
        console.log('Theme applied with inline styles:', theme);
      })();
    `;
