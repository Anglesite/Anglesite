...
async function isTouchIdAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const { stdout } = await execAsync(
      'grep -c "pam_tid.so" /etc/pam.d/sudo 2>/dev/null || echo "0"'
    );
    const touchIdConfigured = parseInt(stdout.trim()) > 0;

    // Also check if Touch ID hardware is available
    const { stdout: biometryCheck } = await execAsync(
      'bioutil -r 2>/dev/null | grep -c "Touch ID" || echo "0"'
    );
...
export async function checkAndSuggestTouchIdSetup(): Promise<void> {
  ...
  if (touchIdAvailable) {
    ...
    console.log(
      "   2. Run: sudo cp /etc/pam.d/sudo_local.template /etc/pam.d/sudo_local"
    );
          ...
          (error: Error | null, stdout: string) => {
            if (error) {
              ...
              if (
                ...
                (error as Error & { code?: number }).code === -128
              ) {
                console.log("Authentication cancelled by user");
              } else if (touchIdAvailable) {
                console.log(
                  "Touch ID authentication failed, may have fallen back to password"
                );
              }
              console.error(
              ...
              if (touchIdAvailable) {
                console.log(
                  "✅ Authentication successful (Touch ID or password)"
                );
              } else {
...
async function addHostEntry(
  ...
  try {
    const result = await executeWithElevatedPrivileges("npx", [
      "hostile",
      "set",
      ipAddress,
      hostname,
...
export async function cleanupHostsFile(): Promise<boolean> {
  try {
    ...
    const hostEntries = await new Promise<Array<[string, string]>>(
      (resolve, reject) => {
        hostile.get(false, (err: Error | null, lines: unknown) => {
          if (err) {
            reject(err);
